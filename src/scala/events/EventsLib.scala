...
import scala.events.IntervalEventFilter
...
trait Event[+T] {
  ...
  /* temporary until compiler does not neet this Join anymore @JVH */
  def join[U, S >: T](other: => Event[U]) = new EventNodeJoin[S, U](this, other)
...
class EventNodeJoin[T, U](ev1: Event[T], ev2: => Event[U]) extends EventNode[(T,U)] {
...
object NoHandler extends scala.Function1[Any,Unit] {
